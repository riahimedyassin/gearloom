...
import { PomodoroTimer as PomodoroTimerType, PomodoroSession, Task } from "../types";
...
const PomodoroTimer: React.FC<PomodoroTimerProps> = ({ task, onUpdateTimer }) => {
  ...
  const handleSessionComplete = () => {
    ...
    const newTotalTime = activeSession.type === 'work' 
    ...
    if (activeSession.type === 'work') {
      const completedWorkSessions = updatedSessions.filter(s => s.type === 'work' && s.completed).length;
      const shouldTakeLongBreak = completedWorkSessions % pomodoroTimer.settings.sessionsUntilLongBreak === 0;
      
      // You could add a notification or auto-start the break here
      console.log(`Work session completed! Time for a ${shouldTakeLongBreak ? 'long' : 'short'} break.`);
    }
  };

  const getNextSessionType = (): 'work' | 'shortBreak' | 'longBreak' => {
    const completedWorkSessions = pomodoroTimer.sessions.filter(s => s.type === 'work' && s.completed).length;
    const lastSession = pomodoroTimer.sessions[pomodoroTimer.sessions.length - 1];
    
    if (!lastSession || lastSession.type !== 'work') {
      return 'work';
    }
    
    return completedWorkSessions % pomodoroTimer.settings.sessionsUntilLongBreak === 0 
      ? 'longBreak' 
      : 'shortBreak';
  };
      ...
      <CardContent className="space-y-4">
        ...
        {activeSession ? (
          <div className="text-center space-y-2">
            <Badge variant={activeSession.type === 'work' ? 'default' : 'secondary'} className="text-sm">
              {activeSession.type === 'work' ? '🍅 Work Session' : 
               activeSession.type === 'shortBreak' ? '☕ Short Break' : '🛋️ Long Break'}
            </Badge>
          ...
          <div className="text-center space-y-4">
            ...
            <div className="flex justify-center gap-2 flex-wrap">
              ...
              <Button onClick={() => startSession('shortBreak')} variant="outline" size="sm">
                <Coffee className="w-4 h-4 mr-1" />
                Short Break
              </Button>
              <Button onClick={() => startSession('longBreak')} variant="outline" size="sm">
                <Coffee className="w-4 h-4 mr-1" />
                Long Break
              </Button>
            </div>
        ...
        <div className="grid grid-cols-2 gap-4 pt-4 border-t">
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600">
              {pomodoroTimer.sessions.filter(s => s.type === 'work' && s.completed).length}
            </div>
